...
class CircuitBreakerSuite extends CatsEffectSuite {
  ...
  def logError(action: String)(err: Throwable, details: RetryDetails): IO[Unit] =
    details match {
      case WillDelayAndRetry(
        ...
        IO {
          println(
            s"$action was failed. So far we have retried $retriesSoFar times. error: ${err.getMessage}"
          )
        }
  ...
  test("When the breaker is open, requests are refused") {
    val program = for {
      ...
      circuitBreaker <- CircuitBreaker
        .create[IO](BreakerOpen(currentTime + 10.seconds), breakerOptions)
    ...
    program
      .attempt
      .map(it =>
        assertEquals(it, Left(CircuitBreakerException(breakerOptions.breakerDescription)))
      )
  }

  test("After a failure happens more than specific times, requests are refused") {
    val program =
      CircuitBreaker.create[IO](breakerOptions.copy(maxBreakerFailures = 1)).flatMap {
        circuitBreaker =>
          retryingOnAllErrors(
  ...
  test(
    "After failures happened asynchronously more than specific times, requests are refused"
  ) {
    val program =
      CircuitBreaker.create[IO](breakerOptions.copy(maxBreakerFailures = 1)).flatMap {
        circuitBreaker =>
  ...
  test(
    "After wait for the specific time during the breaker is opened, request are transported directly again."
  ) {
    ...
    val program = CircuitBreaker
      .create[IO](breakerOptions.copy(maxBreakerFailures = 1, resetTimeoutSecs = RESET_TIME))
